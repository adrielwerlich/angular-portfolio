export const projectsList = [
  // {
  //     "id": "rec2N4vxu050L89Aq",
  //     "fields": {
  //         "Date Published": "2023-07-28",
  //         "Excerpt": "Just a simple game for fun",
  //         "slug": "rocketGame",
  //         "Title": "RocketGame",
  //         "madeWith": "Unity",
  //         "Image": {
  //             "Url": "/assets/projects/images/simple-rocket.png",
  //         },
  //         "Body": "Rocket Game",
  //         "link": "https://sharemygame.com/@adriel/rocketgame",
  //         "language": "en"
  //     }
  // },
  {
    id: 'rec5PRFA0aOtpMPoK',
    fields: {
      'Date Published': '2019-12-15',
      Excerpt:
        'It´s a dice game. Try your luck by arranging the different parameters of the game',
      slug: 'diceGame',
      Title: 'Dice-game',
      madeWith: 'Vue.js',
      Image: {
        Url: '/assets/projects/images/dice-game.png',
      },
      Body: 'Game of dice. It´s possible to use various parameters in order to obtain different results and challenge the player luck to gain some bet. The original version had criptocurrency based bets',
      link: 'https://dice-game.web.app/',
      language: 'en',
    },
  },
  {
    id: 'recDSM9C49yLRBGZn',
    fields: {
      'Date Published': '2023-07-30',
      Excerpt: 'Hit the targets',
      slug: 'shootGame',
      Title: 'ShootGame',
      madeWith: 'Unity',
      Image: {
        Url: '/assets/projects/images/shooter-game.png',
      },
      Body: 'Just shoot the targets',
      link: 'https://play.unity.com/en/games/93ce76d2-679d-4f3b-9591-fd19d16dc18d/very-basic-shooting-game',
      language: 'en',
    },
  },
  {
    id: 'recEr897pCjg353Q9',
    fields: {
      'Date Published': '2020-01-06',
      Excerpt:
        'This is a very ancient tradition born in India five thousand years ago',
      slug: 'krishnaSaid',
      Title: 'Krishna-said',
      madeWith: 'Vue.js and Three.js',
      Image: {
        Url: '/assets/projects/images/krishna.png',
      },
      Body: 'This website presents Krishna and the Bhagavad-gita',
      link: 'http://krishna-said-long-time-ago.web.app/',
      language: 'en',
    },
  },
  //   {
  //     id: 'recIfmK7Xv0SPtJgX',
  //     fields: {
  //       'Date Published': '2023-08-14',
  //       Excerpt: 'Hit the planes',
  //       slug: 'airBattle',
  //       Title: 'AirBattle',
  //       madeWith: 'Unity',
  //       Image: {
  //         Url: '/assets/projects/images/air-battle-game.png',
  //       },
  //       Body: 'Shoot the planes around the world',
  //       link: 'https://sharemygame.com/@adriel/air-battle',
  //       language: 'en',
  //     },
  //   },
  {
    id: 'recNTRM8ChqhQetFf',
    fields: {
      'Date Published': '2020-07-19',
      Excerpt:
        'This website show some 2020 front-end challenges I took up using React.js. Hope you Enjoy it!',
      Title: 'Reactjs-front-end-challenges',
      madeWith: 'React.js',
      Image: {
        Url: '/assets/projects/images/react-app.png',
      },
      Body: 'Created with React.js',
      link: 'https://react-threejs-portfolio.web.app/',
      language: 'en',
    },
  },
  //   {
  //     id: 'recO1C5HChntfNPbB',
  //     fields: {
  //       'Date Published': '2020-11-17',
  //       Excerpt:
  //         'Essa aplicação permite o envio e recebimento de texto entre plataformas',
  //       slug: 'dataExchange',
  //       Title: 'Multi-plataform-data-exchange',
  //       madeWith: 'Flutter',
  //       Image: {
  //         Url: '/assets/projects/images/exchange.png',
  //       },
  //       Body: 'Envio de texto entre plataformas (mobile-desktop)',
  //       link: 'https://login-anantadeva.herokuapp.com/',
  //       language: 'pt',
  //     },
  //   },
  {
    id: 'recQthocPnM4FxuG0',
    fields: {
      'Date Published': '2020-01-07',
      Excerpt:
        'Esssa é uma tradição muito antiga que nasceu na Índia há 5000 anos',
      slug: 'krishnaDisse',
      Title: 'Krishna-disse',
      madeWith: 'Vue.js and Three.js',
      Image: {
        Url: '/assets/projects/images/krishna.png',
      },
      Body: 'Esse site apresenta Krishna e o Bhagavad-gita',
      link: 'http://krishna-disse.web.app/',
      language: 'pt',
    },
  },
  {
    id: 'recRxTLyluCXqNub8',
    fields: {
      'Date Published': '2020-02-18',
      Excerpt:
        'Front end enginnering challenges ever growing new types of solutions and ideas',
      slug: 'frontEndChallenges',
      Title: 'Front-end-challenges',
      madeWith: 'Vue.js',
      Image: {
        Url: '/assets/projects/images/frontend-challenges.png',
      },
      Body: 'Many types of web development project challenges',
      link: 'https://front-end-challenges.web.app/',
      language: 'en',
    },
  },
  {
    id: 'recTd7UHuHaqgQyNY',
    fields: {
      'Date Published': '2019-12-15',
      Excerpt:
        'É um jogo de dados. Tente sua sorte ajustando os diferentes parâmetros do jogo',
      slug: 'diceGame',
      Title: 'Dice-game',
      madeWith: 'Vue.js',
      Image: {
        Url: '/assets/projects/images/dice-game.png',
      },
      Body: 'Jogo de dados. É possível usar vários parâmetros para obter resultados diferentes e desafiar a sorte do jogador para ganhar alguma aposta. A versão original tinha apostas baseadas em criptomoedas',
      link: 'https://dice-game.web.app/',
      language: 'pt',
    },
  },
  {
    id: 'recVozeETIYQP6eWF',
    fields: {
      'Date Published': '2020-07-19',
      Excerpt:
        'Esse site mostra alguns desafios front-end 2020 em usando React.js',
      Title: 'Reactjs-front-end-challenges',
      madeWith: 'React.js',
      Image: {
        Url: '/assets/projects/images/react-app.png',
      },
      Body: 'Feito usando React.js',
      link: 'https://react-threejs-portfolio.web.app/',
      language: 'pt',
    },
  },
  {
    id: 'reca4x4z0noTICLFE',
    fields: {
      'Date Published': '2023-07-30',
      Excerpt: 'Atire nos alvos',
      slug: 'shootGame',
      Title: 'ShootGame',
      madeWith: 'Unity',
      Image: {
        Url: '/assets/projects/images/shooter-game.png',
      },
      Body: 'Atire e acerte',
      link: 'https://play.unity.com/en/games/93ce76d2-679d-4f3b-9591-fd19d16dc18d/very-basic-shooting-game',
      language: 'pt',
    },
  },
  //   {
  //     id: 'recc0eZWIMrqsb9vU',
  //     fields: {
  //       'Date Published': '2023-08-14',
  //       Excerpt: 'Acerte os aviões',
  //       slug: 'airBattle',
  //       Title: 'AirBattle',
  //       madeWith: 'Unity',
  //       Image: {
  //         Url: '/assets/projects/images/air-battle-game.png',
  //       },
  //       Body: 'Atire nos aviões',
  //       link: 'https://sharemygame.com/@adriel/air-battle',
  //       language: 'pt',
  //     },
  //   },
  {
    id: 'recdikkcfJlhXl6j4',
    fields: {
      'Date Published': '2019-10-16',
      Excerpt:
        'Apenas o esqueleto de uma loja virtual que pode ser usada para vender livros, jóias etc.',
      slug: 'haribolStore',
      Title: 'Haribol-store',
      madeWith: 'Vue.js and Laravel',
      Image: {
        Url: '/assets/projects/images/haribol-store.png',
      },
      Body: 'Protótipo para comércio virtual',
      link: 'https://haribol-store.web.app/',
      language: 'pt',
    },
  },
  {
    id: 'receiwuf4RtJqVOxc',
    fields: {
      'Date Published': '2019-09-03',
      Excerpt:
        'Jogo da velha tradicional. Tente vencer a IA ou desligue-a caso não consiga',
      slug: 'hashGame',
      Title: 'Hash-game',
      madeWith: 'Vue.js',
      Image: {
        Url: '/assets/projects/images/tic-tac-toe.png',
      },
      Body: 'Jogo da velha tradicional. Tente vencer a IA',
      link: 'https://adriel-hash-game.web.app/',
      language: 'pt',
    },
  },
  // {
  //     "id": "rech4s0P0N5XZyWze",
  //     "fields": {
  //         "Date Published": "2023-07-28",
  //         "Excerpt": "Simples jogo passatempo ",
  //         "slug": "rocketGame",
  //         "Title": "RocketGame",
  //         "madeWith": "Unity",
  //         "Image": {
  //             "Url": "/assets/projects/images/simple-rocket.png",
  //         },
  //         "Body": "Rocket Game",
  //         "link": "https://sharemygame.com/@adriel/rocketgame",
  //         "language": "pt"
  //     }
  // },
  {
    id: 'rechu9Ilm5LvysM0k',
    fields: {
      'Date Published': '2019-10-16',
      Excerpt:
        'It´s just a skeleton of a possible ecommerce website with products like books, jewels etc',
      slug: 'haribolStore',
      Title: 'Haribol-store',
      madeWith: 'Vue.js and Laravel',
      Image: {
        Url: '/assets/projects/images/haribol-store.png',
      },
      Body: 'Ecommerce prototype',
      link: 'https://haribol-store.web.app/',
      language: 'en',
    },
  },
  //   {
  //     id: 'recjaSATsOWMGDfbT',
  //     fields: {
  //       'Date Published': '2020-11-17',
  //       Excerpt:
  //         'This application allows you to exchange data (text) between your plataforms',
  //       slug: 'dataExchange',
  //       Title: 'Multi-plataform-data-exchange',
  //       madeWith: 'Flutter',
  //       Image: {
  //         Url: '/assets/projects/images/exchange.png',
  //       },
  //       Body: 'Send and receive (exchange) text between your plataforms (mobile / desktop)',
  //       link: 'https://login-anantadeva.herokuapp.com/',
  //       language: 'en',
  //     },
  //   },
  {
    id: 'reckfTVmDicYZ9O6C',
    fields: {
      'Date Published': '2019-09-03',
      Excerpt: 'Traditional hash game. Try to beat the AI, or turn it off',
      slug: 'hashGame',
      Title: 'Hash-game',
      madeWith: 'Vue.js',
      Image: {
        Url: '/assets/projects/images/tic-tac-toe.png',
      },
      Body: 'Traditional hash game. Try to beat the AI, or turn it off',
      link: 'https://adriel-hash-game.web.app/',
      language: 'en',
    },
  },
  {
    id: 'recnc3rbHQcF3shmN',
    fields: {
      'Date Published': '2020-02-18',
      Excerpt:
        'Desafios de desenvolvimento e engenharia de interface de usuário',
      slug: 'frontEndChallenges',
      Title: 'Front-end-challenges',
      madeWith: 'Vue.js',
      Image: {
        Url: '/assets/projects/images/frontend-challenges.png',
      },
      Body: 'Vários tipos de desafios de engenharia de interface de usuário',
      link: 'https://front-end-challenges.web.app/',
      language: 'pt',
    },
  },
  {
    id: 'recpMrXJbYRCllXVH',
    fields: {
      'Date Published': '2019-12-02',
      Excerpt:
        'This app allow for attendants to talk between themselves and to outside clients',
      slug: 'chatSolution',
      Title: 'Chat-solution',
      madeWith: 'Vanilla JS and Firebase',
      Image: {
        Url: '/assets/projects/images/chat-clone.png',
      },
      Body: 'This project was create in order to allow for attendants be registered in connection to a company and departament inside this company. The attendant / operator can talk to other operators and using a WhatsApp API to receive messages from clients talking to the company through the WhatsApp',
      link: 'https://chat-solution.web.app/',
      language: 'en',
    },
  },
  {
    id: 'recz4K9DoZ6ozdIPt',
    fields: {
      'Date Published': '2019-12-02',
      Excerpt:
        'Esse app permite que atendentes conversem entre si e com clientes da empresa',
      slug: 'chatSolution',
      Title: 'Chat-solution',
      madeWith: 'Vanilla JS and Firebase',
      Image: {
        Url: '/assets/projects/images/chat-clone.png',
      },
      Body: 'Este projeto foi criado para permitir que os atendentes sejam registrados em conexão com uma empresa e departamento dentro dessa empresa. O atendente / operador pode conversar com outros operadores e usar uma API do WhatsApp para receber mensagens de clientes que conversam com a empresa por meio do WhatsApp',
      link: 'https://chat-solution.web.app/',
      language: 'pt',
    },
  },
  {
    fields: {
      'Date Published': '2023-11-18',
      Excerpt:
        'This is built with React Three Fiber and React Spring. It´s a 3D portfolio',
      Title: 'React 3D Portfolio',
      madeWith: 'React Three Fiber',
      Image: {
        Url: '/assets/projects/images/3d-port.png',
      },
      Body: 'Since 2021 I started to learn React Three Fiber and React Spring. This is a 3D portfolio built with these technologies. If you click the black door you can play with the dummy or the bettle car',
      link: 'https://r3f-portfolio-three.vercel.app/',
      language: 'en',
    },
  },
  {
    fields: {
      'Date Published': '2023-11-18',
      Excerpt:
        'Isso é construído com React Three Fiber e React Spring. É um portfólio 3D',
      Title: 'React 3D Portfolio',
      madeWith: 'React Three Fiber',
      Image: {
        Url: '/assets/projects/images/3d-port.png',
      },
      Body: 'Desde 2021 comecei a aprender React Three Fiber e React Spring. Este é um portfólio 3D construído com essas tecnologias. Se você clicar na porta preta você pode brincar com o boneco ou com o carro bettle',
      link: 'https://r3f-portfolio-three.vercel.app/',
      language: 'pt',
    },
  },
  {
    fields: {
      'Date Published': '2021-06-12',
      Excerpt:
        'This is built with Vue and Three.js library for 3D effects on the web',
      Title: 'Vue 3D Portfolio',
      madeWith: 'Vue.js e Three.js',
      Image: {
        Url: '/assets/projects/images/vue-port.png',
      },
      Body: 'My first 3D portfolio. It was built with Vue.js and Three.js. It has some 3D effects demonstrating some examples of what is possible to do on the Web with modern technologies',
      link: 'https://adriel-portf.web.app/',
      language: 'en',
    },
  },
  {
    fields: {
      'Date Published': '2021-06-12',
      Excerpt:
        'Isso é construído com React Three Fiber e React Spring. É um portfólio 3D',
      Title: 'Vue 3D Portfolio',
      madeWith: 'Vue.js and Three.js',
      Image: {
        Url: '/assets/projects/images/vue-port.png',
      },
      Body: 'Meu primeiro portfólio 3D. Foi construído com Vue.js e Three.js. Tem alguns efeitos 3D demonstrando alguns exemplos do que é possível fazer na Web com tecnologias modernas',
      link: 'https://adriel-portf.web.app/',
      language: 'pt',
    },
  },
  {
    fields: {
      'Date Published': '2023-11-19',
      Excerpt: 'This is my first flutter app published on the web 😁',
      Title: 'Flutter Portfolio',
      madeWith: 'Flutter',
      Image: {
        Url: '/assets/projects/images/flutter-port.png',
      },
      Body: "This is the first app I've built with Flutter. It's a portfolio app. It's possible to see some of my projects and contact me.",
      link: 'https://adriel-flutter-portfolio.vercel.app/',
      language: 'en',
    },
  },
  {
    fields: {
      'Date Published': '2023-11-19',
      Excerpt: 'Este é meu primeiro aplicativo flutter publicado na web 😁',
      Title: 'Flutter Portfolio',
      madeWith: 'Flutter',
      Image: {
        Url: '/assets/projects/images/flutter-port.png',
      },
      Body: 'Este é o primeiro aplicativo que desenvolvi com Flutter. É um aplicativo de portfólio. É possível ver alguns dos meus projetos e entrar em contato comigo.',
      link: 'https://adriel-flutter-portfolio.vercel.app/',
      language: 'pt',
    },
  },
];

interface Project {
  id: string;
  fields: {
    'Date Published': string;
    Excerpt: string;
    slug: string;
    Title: string;
    Image: {
      Url: string;
    };
    Body: string;
    link: string;
    language: string;
  };
}
declare module './Projects' {
  const projects: Project[];
}
